...
trans_group = TransactionGroup(doc, 'Align Level Beam to Girder')
trans_group.Start()
...
t1 = Transaction(doc, 'Đổi level giằng theo dầm dốc')
t1.Start()
...
t2.Start()
set_start = module.get_builtin_parameter_by_name(beam, DB.BuiltInParameter.STRUCTURAL_BEAM_END0_ELEVATION)
set_end = module.get_builtin_parameter_by_name(beam, DB.BuiltInParameter.STRUCTURAL_BEAM_END1_ELEVATION)

set_start_Z = module.get_builtin_parameter_by_name(beam, DB.BuiltInParameter.START_Z_OFFSET_VALUE)
set_end_Z = module.get_builtin_parameter_by_name(beam, DB.BuiltInParameter.END_Z_OFFSET_VALUE)

set_start_Z.Set(set_start.AsDouble())
set_end_Z.Set(set_end.AsDouble())
set_start.Set(1)
t2.Commit()

t3 = Transaction(doc, 'Đổi level giằng theo dầm dốc')
t3.Start()
set_start.Set(0)
set_start.Set(0)
t3.Commit()
