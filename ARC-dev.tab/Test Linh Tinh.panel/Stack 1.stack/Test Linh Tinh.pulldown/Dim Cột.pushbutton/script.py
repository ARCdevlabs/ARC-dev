...
if module.AutodeskData():
    ...
    def get_symbol_or_instance_geometry(element):
        if element.HasModifiedGeometry():
            option = Options()
            option.ComputeReferences = True
            get_instance_geometry = element.get_Geometry(option)
            ...
            if geometry_element is not None:
                for geom_object in geometry_element:
                # Check if the geometry object is a GeometryInstance
                    if isinstance(geom_object, GeometryInstance):
                        trans = element.GetTransform()

                        # symbol_geometry = geom_object.GetInstanceGeometry() 
                        symbol_geometry = geom_object.GetSymbolGeometry()
                        # print geometry_element.Volume
                        # print symbol_geometry.GetTransformed(instance_transform)
                        # print symbol_geometry, dir(symbol_geometry), symbol_geometry.GetTransformed(instance_transform)
        ...
        for geometry_object in geometry:
            if hasattr(geometry_object, "Faces"):
                if geometry_object.Volume != 0:
                    print dir(geometry_object)
                    print "Đây là" + str(geometry_object)
                    for face in geometry_object.Faces:
                    # print dir(face), face.IsElementGeometry
        ...
        with revit.Transaction("Column Dimension", swallow_errors=True):
            for column in Ele:
                ...
                try:
                    ...
                    geo = get_symbol_or_instance_geometry(column)
                    faces = get_face(geo)
                    print faces
                    X_vector =get_X_vector(column)
                    Y_vector = get_Y_vector(column)
                    ...
                    try:
                        ...
                        detail_line = doc.Create.NewDetailCurve(Currentview,new_line_for_dim_total_X)
                        detail_line = doc.Create.NewDetailCurve(Currentview,new_line_for_dim_center)
                    except:
                        import traceback
                        print(traceback.format_exc())
                        pass
                    ...
                    print dim_center.Id,dim_center_X.Id
