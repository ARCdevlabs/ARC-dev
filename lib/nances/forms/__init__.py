...
class ARCSelectForm(TemplateUserInputWindow):

    xaml_source = 'ARCSelectForm.xaml'
    ...
    def _setup(self, **kwargs):
        ...
        if kwargs.get('multiselect', False):
            self.multiselect = True
            self.list_categoy.SelectionMode = Controls.SelectionMode.Extended
            self.show_element(self.checkboxbuttons_g)
        else:
            self.multiselect = False
            self.list_categoy.SelectionMode = Controls.SelectionMode.Single
            self.hide_element(self.checkboxbuttons_g)
    ...
    def _get_options(self):
        if self.multiselect:
            if self.return_all:
                ...
                return self._unwrap_options(
                    [x for x in self._get_active_ctx()
                     if x.state or x in self.list_categoy.SelectedItems]
                    )
        else:
            return self._unwrap_options([self.list_categoy.SelectedItem])[0]

    def _set_states(self, state=True, flip=False, selected=False):
        if selected:
            current_list = self.list_categoy.SelectedItems
        else:
            current_list = self.list_categoy.ItemsSource
        for checkbox in current_list:
            # using .checked to push ui update
            if flip:
                checkbox.checked = not checkbox.checked
    ...
    def button_reset(self, sender, args):#pylint: disable=W0613
        if self.reset_func:
            all_items = self.list_categoy.ItemsSource
            self.reset_func(all_items)
    ...
    def selected_item_changed(self, sender, args):
        if self.info_panel and self.list_categoy.SelectedItem is not None:
            self._toggle_info_panel(state=True)
            self.infoData.Text = \
                getattr(self.list_categoy.SelectedItem, 'description', '')
...
def select_views_ARC(title='Select Views',
                 button_name='Select',
                 width=DEFAULT_INPUTWINDOW_WIDTH,
                 multiple=True,
                 filterfunc=None,
    ...
    selected_views = ARCSelectForm.show(
        sorted([ViewOption(x) for x in all_graphviews],
               key=lambda x: x.name),
        title=title,
        button_name=button_name,
